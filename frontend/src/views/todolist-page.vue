<template>
  <div>
    <b-card id="todolistPage" class="my-3">
      ...
      <b-button title="About" :to="{ name: 'home' }"
        ><font-awesome-icon :icon="['fas', 'home']" class="mr-1" />
        Home
      </b-button>
      <b-button title="About" :to="{ name: 'about' }" class="ml-2"
        ><font-awesome-icon :icon="['fas', 'info']" class="mr-1" />
        About
      </b-button>
    </b-card>
    <div>
    ...
    <b-card
      ref="todoCard"
      class="my-2"
      v-for="(todo, index) in myTodoList"
      :key="index"
    >
      <b-row>
        <b-col class="col-9">
    ...
    <TodoModal
      :is-show-modal="modalShow"
      :title="modalTitle"
      v-model="todo"
      @create:success="onCreateSuccess"
      @update:success="onUpdateSuccess"
      @hidden="(event) => closeModal(event)"
